...
{
       register u32 r0 __asm("r0") = smc_arg;
        __asm volatile(
...
/**
 * @brief SMC is called non-secure kernel is invoked 
 */
void invoke_ns_kernel(void)
{
    register u32 r0 __asm("r0") = INVOKE_NON_SECURE_KERNEL;
    do {
...
/**
 * @brief return from secure api
 *
 * @param retval
 */
void return_secure_api(u32 retval)
{
    register u32 r0 __asm("r0") = RET_FROM_SECURE_API;
...
/**
* @brief non-secure cpu context initialization 
*/
void smc_nscpu_context_init(void)
{
    register u32 r0 __asm("r0") = TZHYP_NSCPU_CTXT_INIT;
    do {
