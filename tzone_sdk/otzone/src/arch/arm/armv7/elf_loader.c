	...
	for(i=1; i<n ; i++){
		...
		if(sw_strcmp((char *)(strtab + index), entry_func) == 0){
			...
			psa_config->process = (int(*)(u32,void*,u32,void*,u32, 
				struct otzc_encode_meta *,u32*))(BASE_LOAD_ADDRESS 
				 				+ sym[i].st_value); 
			return 0;
		}
		...
		int *fl)
{
	...
	for(i=0; i < (rel_sec->sh_size)/(sizeof(Elf32_Rel)); i++){
		...
		if( ((int)rel[i].r_offset < 0 || rel[i].r_offset > 
 ...
 */
static int get_number_of_loadable_segments(int fp, Elf32_Ehdr *elf_header, 
		Elf32_Phdr **phdr_tab)
{
	int ph_num,num_of_segments,num_of_loadable_segments = 0;
...
int elf_load( struct sa_config_t *conf)
{
	...
	Elf32_Ehdr *tmp_ehdr;
	int rel = 0,status,num_of_loadable_segments = 0;
	u32 num_bytes = 0,va_offset;
	int num_of_segments,fp = -1,ret = OTZ_OK,mode = FILE_READ;
	u32 *flag = NULL,*base_va = NULL;
	u32 offset,leftover_size;	Elf32_Phdr *phdr_table = NULL, *tmp_phdr = NULL;
