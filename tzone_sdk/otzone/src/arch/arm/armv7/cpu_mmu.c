...
void set_domain(u8 domain, access_type access)
{
    ...
    */
    ...
    u32 mask = ~(__extension__ 0b11 << (domain*2));
    value = value & mask;
    //Set the domain
...
{
    pa_t pa;
    __asm volatile("mcr p15, 0, %0, c7, c8, 0\n"::"r"(va):"memory", "cc");
...
{
    pa_t pa;
    __asm volatile("mcr p15, 0, %0, c7, c8, 4\n"::"r"(va):"memory", "cc");
...
void cpu_mmu_enable(void)
{
    ...
    mmu_init();
    set_ttbcr(0);
    mmu_insert_pt0((u32)pagetable_addr);
