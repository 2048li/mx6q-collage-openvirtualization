					   ...
					   u8 *dir_offset,const char *path)
{
	...
	finfo->file_path = sw_malloc(sizeof(char) * (sw_strlen((char*)path) + 1));
	sw_strcpy((char*)finfo->file_path,(char*)path);
	finfo->entry = entry;
...
int file_read(int fd,char * buffer,int bytes_to_read)
{
	...
	if(temp->entry->attr != ATTR_DIR){
		tmp_offset = (u32)bytes_to_read > (temp->file_size - temp->bytes_traversed) ?
		     	     temp->file_size - temp->bytes_traversed : (u32)bytes_to_read; 
...
int file_write(int fd, const char *buffer,int bytes_to_write)
{
	file_info *temp = file_head;
	u8 *file_cont;
	u32 tmp_offset = bytes_to_write;
	u32 alloc_cluster_cnt = 0;
	u32 bytes_to_copy;
	u32 bytes_written = 0;
	u32 new_cluster;
	u32 prev_cluster;	
...
int file_close(int fd)
{
	...
	if(temp == file_head){
		file_head = temp->next;
		sw_malloc_free((void*)temp->file_path);
...
int create_file(char *lfname,char *shrt_name,u32 clus,dir_entry **entry)
{
	...
	while(!is_gap_found){
		...
		if(lfname != NULL){
			chk_sum = calc_check_sum((u8*)shrt_name);
			while(*lfptr != '\0'){
				lfptr++;
				i++;
			}
...
void fill_lng_fname_entries(char *fname,u8 *entry,u8 chk_sum,int count)
{
	...
	int max_ord_cnt;
	int ord_num = 1;
	lfn_ent = (lfn_entry *)(lfn_ent - DIR_ENTRY_LEN);
	while(count--){
		lf_entry = (u8 *)lfn_ent;
...
file_stat * read_dir(int fd)
{
	...
	*(sfn + (index + 1)) = '\0';
	...
	if(*lfn != SPACE_VAL){
		...
		sw_strcpy((char*)stat->lname,lfn);
		sw_strcpy(stat->name,(char*)entry->name);
	}
	else{
		sw_strcpy(stat->name,sfn);
		stat->lname = (char*)SPACE_VAL;
	}
...
{ 
	...
	u32 bytes_traversed = 0;
	u32 bytes_to_copy;

	if(((u32)offset + temp->cur_offset < cluster_size) && strt_cluster != 0){
...
int cal_req_dir_cnt(int file_name_len)
{
	...
	req_cnt = len != 0 ? (req_cnt+1) : req_cnt;
	return req_cnt;
}
...
int file_truncate(const char* tr_pf,int tr_lgh)
{
	...
	int sk_ted,end_size,tmp_tsize,wt_zo=0,zo_cnt=0;
	char  buff[1024]={'\0'};
