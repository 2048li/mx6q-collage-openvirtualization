...
/* 
 * Crypto related tasks implementation
 * Hasher algorithm, HMAC, Cipher algorithm tasks
 */
...
/**
* @brief Adds all digest and encryption algorithms to internal table 
*/
...
/**
* @brief Removes all ciphers and digests from the table
*/
...
/**
* @brief MD5 and SHA Algorithm tests
*
* @param req_buf
* @param req_buf_len
* @param res_buf
                ...
                u32 *ret_res_buf_len,u32 svc_cmd_id)
{
    ...
    u8 *out_buf;
    int offset = 0, pos = 0, mapped = 0, type =0, out_len =0;
	u32 cmd_id = 0;
    ...
    switch(svc_cmd_id) {
    	...
    	case OTZ_CRYPT_CMD_ID_SHA512:
    		...
    		if(test_digest_api((u8*)crypto_data.data,out_len,
                    		(u8*)crypto_data.response,(u32*)&(crypto_data.len),
                    			cmd_id) != TEEC_SUCCESS) {
                ...
                u32 *ret_res_buf_len,u32 svc_cmd_id)
{
    ...
    switch(svc_cmd_id) {
    	...
    	case OTZ_CRYPT_CMD_ID_HMAC_SHA512:
    		...
    		if(test_hmac_api((u8*)crypto_data.data,out_len,key_buf,key_len,
                    		(u8*)crypto_data.response,(u32*)&(crypto_data.len),
                    			cmd_id) != TEEC_SUCCESS) {
    			sw_memcpy(crypto_data.response,crypto_data.data,out_len);
    			crypto_data.len = out_len;
    		}
    		if(compare_hmac_values((u8*)crypto_data.data,out_len,key_buf,key_len,
                    		(u8*)crypto_data.response,(u32*)&(crypto_data.len),
                    				cmd_id) != TEEC_SUCCESS) {
    			sw_memcpy(crypto_data.response,crypto_data.data,out_len);
    			crypto_data.len = out_len;
                ...
                u32 *ret_res_buf_len,u32 svc_cmd_id)
{
    ...
    u32 cipher_choice = 0;
    u16 key_len = 0,init_vector_len=0;
    int offset = 0, pos = 0, mapped = 0, type =0, out_len =0;
    ...
    switch(svc_cmd_id) {
        ...
        case OTZ_CRYPT_CMD_ID_CIPHER_DES3_CBC:
    		...
    		if(test_cipher_api((u8*)crypto_data.data,out_len,local_init_vector,
...
#ifdef _CRYPTO_BUILD_
	psa_config->process = (int (*)(u32,void*,u32,void*, u32, 
					struct otzc_encode_meta *, u32*))&process_otz_crypto_svc;
#endif
