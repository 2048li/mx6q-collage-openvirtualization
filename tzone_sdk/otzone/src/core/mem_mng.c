...
pa_t sw_phy_page_alloc(u32 size )
{
    ...
    for(start = 0; 
        (u32)start < ((sw_mem_info.phy_mem_bmap_len << 3) - req_cnt);
        start++)
    {
        avl_cnt = 0;
...
u32* sw_vir_page_alloc(u32 size )
{
    ...
    for(start = 0; (u32)start < ((sw_mem_info.vir_mem_bmap_len << 3) - req_cnt);
                                                                     start++)
    {
        avl_cnt = 0;
        for(iter = start; iter < ( start + req_cnt) ; iter++){
...
/**
* @brief Reserves the required pages in the bitmap
*
* @param vir_addr : virtual address of the area to be reserved 
* @param size : size of the area to be reserved 
*
