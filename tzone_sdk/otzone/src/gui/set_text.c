...
void set_text(char* str, u8 id[])
{
	...
	while(countt!=-1) {
		if(sw_strcmp((char *)obj_val.obj_id[countt], (char*)id) == 0) {
			break;
		}
		countt--;
	}
	...
	if(countt != -1) {
		...
		if(sw_strcmp((char*)obj_val.obj_type[countt], "textfield\0") == 0) {
			...
			put_string(obj_val.obj_coord[countt][0]+10, 
					obj_val.obj_coord[countt][1]+(height-8)/2, (u8*)str, 
					sw_strlen(str),
					calc_contrast_color(obj_val.obj_color[countt]), NO_TEXT_BG);
			sw_strcpy((char*)obj_val.obj_str[countt], str);
		}

		if(sw_strcmp((char*)obj_val.obj_type[countt], "password\0") == 0) {
			fill_rect(obj_val.obj_coord[countt][0],obj_val.obj_coord[countt][1],
					obj_val.obj_coord[countt][2], obj_val.obj_coord[countt][3],
					obj_val.obj_color[countt]);
			...
			if(sw_strcmp(str, "\0") != 0) {
				...
				sw_strcpy((char*)obj_val.obj_str[countt], (char*)temp_str);
				sw_strcpy(str, (char*)temp_str);
			}
			else {
				put_string(obj_val.obj_coord[countt][0]+10, 
						obj_val.obj_coord[countt][1] + (height-8)/2, (u8*)str, 
