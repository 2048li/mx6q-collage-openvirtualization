		...
		s32int height, u32 *bmp_array, char* id)
{
	...
	set_object_values_given_coord((u8*)id, (u8*)"", x, y, width, height, (u8*)"bmp\0", 0);
}
...
void bitmap(s32int width, s32int height, u32* bmp_array, char* id)
{
	...
	if(!obj_val.end_inserting_obj) {
		if(check_space_for_object((u8*)"", width, height)) {
			...
			set_object_values((u8*)id, (u8*)"", width, height, (u8*)"bmp\0", 0);
		}
	}
}
