...
* @param color
*/
void label(u8* id, u8* str, s32int width, s32int height, u32 color)
{
	if(!obj_val.end_inserting_obj) {
		compute_label_width(str);
		...
		if(check_space_for_object(str, width, height)) {
			...
			put_string(obj_val.obj_x+(sw_strlen((char*)str)
						+(width-(sw_strlen((char*)str)*10))/2), 
					obj_val.obj_y + (height-8)/2, str, sw_strlen((char*)str),
					calc_contrast_color(color), NO_TEXT_BG);

			set_object_values(id, str, width, height, (u8*)"label\0", color);
		}
	}
}
...
* @param col
*/
void label_on_given_coord(s32int x, s32int y, u8* str, u8* id, s32int width, 
		s32int height, u32 col)
{
	fill_rect(x, y, x+width, y+height, color(GRAY));
	fill_rect(x+3, y+3, x+width, y+height, col);

	put_string(x+(sw_strlen((char*)str)+(width-(sw_strlen((char*)str)*10))/2),
			y + (height-8)/2, str, sw_strlen((char*)str), calc_contrast_color(col),
			NO_TEXT_BG);
	set_object_values_given_coord(id, str, x, y, width, height, (u8*)"label\0", col);
}
