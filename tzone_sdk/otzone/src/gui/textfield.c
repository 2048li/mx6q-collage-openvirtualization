	...
	if(!obj_val.end_inserting_obj) {
		...
		if(check_space_for_object(str, width, height)) {
			...
			if(password == NO_PASSWORD)
				set_object_values(id, str, width, height, (u8*)"textfield\0", color);
			else
				set_object_values(id, str, width, height, (u8*)"password\0", color);
		}
	}
}
...
/*
	sw_printf("TEXTBOX:%s\n", str);
*/
	put_string(x + 10, y + (height-8)/2, str, sw_strlen((char*)str),
