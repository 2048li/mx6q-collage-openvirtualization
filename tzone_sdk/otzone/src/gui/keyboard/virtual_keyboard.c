...
/**
* @brief Sets the Keyboard window and Draws the bitmap image of  
*		all the keys of the virtual keyboard
* @param vk_active
*
* @return 
*/
s32int virtual_keyboard(s32 vk_active)
{
...
#endif
	vk_close = 0;
	set_window(0, 672, 600, 1024, (u8*)"VIR KEYBOARD\0", color(BLACK), NO_TITLE_BAR);
	...
	label((u8*)"label1\0", (u8*)"", KEY_WIDTH/2, KEY_HEIGHT/2, color(BLACK));
...
/**
* @brief On touch of a key in the keyboard, the key changes it colour by
*		drawing the keyon bitmaps over the existing keyboard keys image  
*/
void on_touch(void)
{
#ifdef DEBUG_VK
...
/**
* @brief On release of the key the default key bitmap is loaded again and 
*		the key pressed is stored in an array 
*/
void on_release(void)
{
	...
	else if(sw_strcmp((char*)touch_info.last_touched, "l\0") == 0) {
		sw_strcat((char*)clicked_text[vk_active], "l\0");
		calc_bmp_array_on_given_coord(touch_info.x, touch_info.y, KEY_WIDTH, 
	...
	else if(sw_strcmp((char*)touch_info.last_touched, "space\0") == 0) {
		sw_strcat(clicked_text[vk_active], " \0");
		calc_bmp_array_on_given_coord(touch_info.x, touch_info.y, 209, 
