...
/**
* @brief calculates the bitmap array based on the data pointer  
*		and draws the bitmap for the given width and height
* @param width
* @param height
* @param data
* @param str
...
void calc_bmp_array_on_given_coord(s32int x, s32int y, s32int width, 
		s32int height, char* data, char* str)
{
	s32int i = width * height, k = 0, j = 0;
	while(i-- > 0) {
		pixel[k+3] = ((((data[2] - 33) & 0x3) << 6) | ((data[3] - 33)));
...
/**
* @brief customize the bitmap based on the orientation and  
*		set pixel for the window
* @param x
* @param y
* @param width
* @param height
