...
#if _DEBUG
#define OTZ_DPRINTK(...)   sw_printf(__VA_ARGS__)
#else
#define OTZ_DPRINTK(...)
#endif
...
/**
 * @brief This function is used to get the next free house keeping node address 
 *
 * @return 
 */
...
/**
 * @brief Frees the house keeping memory region at the requested address node 
 *
 * @param node
 */
...
/**
 * @brief Gets the free node in the allocated memory space 
 *
 * @return 
 */
...
/**
 * @brief Buddy allocator initialization
 *
 * @param index
 * @param heap_start
 * @param heap_size
...
int buddy_init(int index, void *heap_start, unsigned int heap_size)
{
    ...
    buddy_heap[index].hk_an_array = 
            (struct sw_alloced_area *)((int)heap_start + (hk_total_size / 2));
    ...
    mem_start = (void*)((int)heap_start + hk_total_size);
    ...
    while (mem_size > 0) {
        ...
        mem_start = (void*)((u32)mem_start + MAX_BLOCK_SIZE);
...
/**
 * @brief Gets the contiguous block of memory space at the requested heap index 
 *			for the num_blocks contiguous memory blocks
 * @param num_blocks
 * @param idx
 *
 * @return 
    ...
    if (buddy_heap[index].free_area[idx].count >= num_blocks) {
        ...
        list_for_each(cnhead, &buddy_heap[index].free_area[idx].head) {
            ...
            if (cnode) {
                ...
                if (pnode) {
                    if ((void*)((int)pnode->map + MAX_BLOCK_SIZE) ==
                        (void*)(int)cnode->map) {
                        pnode = cnode;
                        if (++count == num_blocks) {
...
/**
 * @brief Gets a block of memory at the requested heap index 
 *
 * @param idx
 *
 * @return 
    ...
    if (list_empty(&buddy_heap[index].free_area[idx].head)) {
        ...
        if (farea) {
            ...
            if (rarea) {
                ...
                rarea->map = (void*)((int)farea->map + blk_sz);
            }
        }
    } else {
...
/**
 * @brief This function searches whether the memory has been allocated in the particular
 *		heap index and address specified 
 * @param index
 * @param addr
 *
 * @return 
...
/**
 * @brief Adds the free area passed to the heap index at the specified bin_num 
 *
 * @param free_area
 * @param bin_num
 *
...
/**
 * @brief Combining memory blocks so that to get bigger contiguos block of memory 
 *
 * @param index
 * @param bin
 *
...
restart:
    ...
    list_for_each(pos, &buddy_heap[index].free_area[bin].head) {
        ...
        if (lmap) {
            if (((void*)((int)lmap + (MIN_BLOCK_SIZE << bin))) == cfa->map) {
                OTZ_DPRINTK
...
/**
 * @brief Returns the freed area to the heap index for future memory allocations
 *
 * @param aarea
 *
 * @return 
...
/**
 * @brief allocates memory at the requested heap index for size bytes 
 *
 * @param index
 * @param size
 *
...
{
    int idx = 0;
    unsigned int curr_blk = MIN_BLOCK_SIZE;
...
/**
 * @brief Frees the allocated memory block at the given heap index and address ptr 
 *
 * @param index
 * @param ptr
 */
...
/**
 * @brief Prints the state of the heap allocations
 *		Whether heap is found for this task 
 * 		Heap size, no. of free and allocated blocks 
 */
...
/**
 * @brief Prints the status of house keeping region of heap memory
 *		No. of free and allocated nodes in the heap 
 */
...
/**
* @brief Allocates memory privately for size bytes at the memory related to heap_id passed  
*
* @param heap_id
* @param size
*
...
/**
 * @brief Gets the task id and allocates memory in the heap memory 
...
/**
 * @brief Frees the privately allocated memory pointing to the  
 *			heap_id and the address pointer
 * @param heap_id
 * @param pointer
 */
...
/**
 * @brief Gets the task_id and frees the heap memory corresponding 
 *			to the task_id
 * @param pointer
 */
...
/**
* @brief Allocates heap memory initially during task creation for a particular heap_id  
*		Checks for already allocated heap matching the heap_id,
*		If no memory was allocated earlier, memory is allocated for the 
*		passed heap_id
* @param heap_id
*
* @return
...
/**
* @brief Frees the private heap already allocated corresponding to 
*			the heap_id 
*
* @param heap_id
*
...
/**
* @brief Gets the pointer size of the allocated block of memory 
*
* @param ptr
*
* @return 
