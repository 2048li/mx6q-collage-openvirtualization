...
static s32int sw_clear_stream(FILE *instream)
{
    ...
    write_count = f_write((int)instream->_fdesp,instream->_buf_base,
							(int)num_items);
    instream->_chr_count = 0;
...
static s32int sw_occupy_buffer(FILE *stream)
{
    ...
    stream->_chr_count = f_read((int)stream->_fdesp, stream->_chr_ptr, 
     ...
     if(strm->_flags & SW_IOLBF) {
        *strm->_chr_ptr++=cont;
        if(cont=='\n' || strm->_chr_count==-strm->_buf_size) {
            ...
            strm->_chr_ptr=strm->_buf_base;
            strm->_chr_count=0;
             if(!f_write(strm->_fdesp,(strm->_buf_base),sw_count)) {
        ...
        u8 *c_val=(u8*)cont;
        strm->_chr_count=0;
        if (f_write(strm->_fdesp,(unsigned char *)&c_val,1)!=1) {
            strm->_flags|= SW_ERROR;
#ifdef FILE_LIB_DBG
        ...
        if(fw_count>0) {
            if(!f_write(strm->_fdesp,(strm->_buf_base),fw_count)) {
                *(strm->_buf_base)=cont;
                strm->_flags|= SW_ERROR;
#ifdef FILE_LIB_DBG
                sw_printf("the value of strm_flag is %x \n",strm->_flags);
...
/**
* @brief function shall open the file whose pathname is the string pointed to by 
...
/**
* @brief Reads an array of count elements from an inputstream
*
* @param out_buf - Pointer to a block of memory with a size of at least 
*				   (size*count) bytes
* @param size - Size in bytes, of each element to be read
* @param count - Number of elements, each one with a size of size bytes
* @param inputstream - Pointer to a FILE object that specifies an input stream
*
...
#endif
    w_count=(s32int)(f_write(pfile->_fdesp,(unsigned char *)content,(int)count));
    if(w_count==0) {
