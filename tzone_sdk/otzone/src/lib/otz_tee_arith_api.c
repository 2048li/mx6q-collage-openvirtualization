...
#include <unused.h>
/**
 * @brief returns the size of the array of uint32_t values needed to
 *	represent an integer in the fast modular multiplication representation
 * 	given the size of the modulus in bits
 *
...
size_t TEE_BigIntFMMSizeInU32(size_t moduleSizeInBits){
    /*Not implemented*/
    UNUSED(moduleSizeInBits);
    return TEE_ERROR_NOT_IMPLEMENTED;
}

/**
 * @brief Returns the size of the array of uint32_t values
 *	needed to represent a fast modular context using a given modulus size
 *
 * @param ModuleSizeInBits
 *
 * @return 
...
/**
 * @brief Initializes bigInt and sets its represented value to zero. 
 *	This function assumes that bigInt points to a memory area of len uint32_t
 *
 * @param bigInt
 * @param len
 *
...
/**
 * @brief Initializes bigIntFMM and sets its represented value to zero. 
 *	This function assumes that bigIntFMM points to a memory area of len uint32_t
 *
 * @param bigIntFMM
 * @param len
 *
...
/**
 * @brief Converts a bufferLen byte octet string buffer into
 *		a TEE_BigInt format
 *
 * @param dest
 * @param buffer
...
/**
 * @brief Converts the absolute value of an integer in
 *	TEE_BigInt format into an octet string. 
 *	The octet string is written in a most significant byte first representation.
 *
 * @param buffer
 * @param bufferLen
 * @param bigInt
 ...
 *	If src does not fit within an int32_t, the value of *dest is undefined
 *
 * @param dest
 * @param src
 *
...
/**
 * @brief Checks whether op1>shortVal, op1==shortVal, or op1<shortVal
 *
 * @param op1
 * @param shortVal
 *
...
int TEE_BigIntCmpS32(TEE_BigInt* op1, int shortVal){
    ...
    ptr1 = op1; ptr2 = (u32*)&shortVal;
    for(i = 0; i < (int)(len - 1 - (sizeof(int)/sizeof(TEE_BigInt))); i++){
        if(*(ptr1+i) != 0){
            ret_val = 1;
        }
    }
...
/**
 * @brief Computes |dest| = |src| >> bits and dest will have the
 *		same sign as op
 *
 * @param dest
 * @param src
...
/**
 * @brief Negates an operand: dest = -src
 *
 * @param dest
 * @param src
 */
...
/**
 * @brief Returns the bitIndexth bit of the natural binary representation of |src|.
 *	A true return value indicates a “1” and a false return value 
 *			indicates a “0” in the bitIndexth position
 *
 * @param src
 * @param bitIndex
 *
...
/**
 * @brief Computes dest_r and dest_q such that op1 = dest_q * op2 + dest_r. 
...
/**
 * @brief Computes dest = op1 * src (mod n)
 *
 * @param dest
 * @param op1
 * @param src
...
/**
 * @brief Computes dest = src * src (mod n)
 *
 * @param dest
 * @param src
 * @param n
...
/**
 * @brief Computes dest such that dest * op = 1 (mod n)
 *
 * @param dest
 * @param op
 * @param n
...
/**
 * @brief Determines whether gcd(op1, op2)==1
 *
 * @param op1
 * @param op2
 *
...
/**
 * @brief Computes the greatest common divisor of the input
 *					parameters op1 and op2. 
 *Furthermore it computes the coefficients u and v such that u*op1+v*op2==gcd
 *
 * @param gcd
 * @param u
 * @param v
...
/**
 * @brief The TEE_BigIntIsProbablePrime function performs a probabilistic primality test on op. 
 *	The parameter confidenceLevel is used to specify the probability of a non-conclusive answer. 
 *	If the function cannot guarantee that op is prime or composite, 
 *	it MUST iterate the test until the probability that op is composite is
 *	less than 2^(-confidenceLevel). Values smaller than 80 for confidenceLevel will not be recognized
 *	and will default to 80. The maximum honored value of confidenceLevel is 
 *	implementation specific, but MUST be at least 80.
 *
 * @param op
