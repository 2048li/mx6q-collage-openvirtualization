...
/**
 * @brief Before a device can be used permission has to be sought  
 *		This function gets permission pertaining to the requested device id 
 *		and it returns 1 if permission is allowed 
...
/**
* @brief Sets permission for the task passed  
*
* @param new_task
*
* @return 
...
/**
 * @brief Updates the global access control for the given device id or service id  
 *
 * @param d_id device id
 * @param g_id group id 
 * @param u_id user id 
...
void updateGlobalACL(int d_id, int g_id, int u_id){  
    ...
    list_for_each_entry(acld, &global_val.device_acl_list, head){
      if(acld->did == (u32)d_id){
          found = 1;
          list_for_each_entry(aclg, &acld->group_head, head){
              if((u32)g_id == aclg->gid){
                  found = 2;
                  list_for_each_entry(aclu, &aclg->user_head, head){
                      if((u32)u_id == aclu->uid){
                          found = 3;
                          break;
                      }
                  }
