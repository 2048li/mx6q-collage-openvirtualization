...
/**
 * @brief Writes characters onto the print buffer
 *
 * @param print_buffer
 */
...
/**
 * @brief Dummy UART device IOCTL call
 */
...
static void __init uart_dev_init(void)
{
    ...
    updateGlobalACL(DEVICE_ID,1,1);
    updateGlobalACL(DEVICE_ID,1,2);
    updateGlobalACL(DEVICE_ID,1,3);
    updateGlobalACL(DEVICE_ID,1,4);
    updateGlobalACL(DEVICE_ID,1,5);
    serial_puts("\n\rSW: UART driver initialized successfully\r\n");
    return;
}

/**
 * @brief Unregisters the File operations structure of UART
 */
