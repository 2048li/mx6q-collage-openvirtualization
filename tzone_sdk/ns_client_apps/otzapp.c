...
int perform_echo()
{
    ...
    strncpy_safe(echo_data.data, "test data abcdefgh",DATA_BUF_LEN);
...
%s and out data len 0x%x\n", (shared_mem.p_block + 512), 
    ...
    strncpy_safe(alloc_buf, "test_array_space_buffer",sizeof(alloc_buf));
    ret = otz_operation_perform(&operation_otz ,&service_ret);
    if(ret != OTZ_SUCCESS) {
        if(ret == OTZ_ERROR_SERVICE)
            printf("%s \n",otz_strerror(service_ret));
...
int test_otz_mutex()
{
    ...
    memncpy(otz_mutex_test_data.data,input_buf,input_buf_len,
			CRYPT_BUF_LEN, CRYPT_BUF_LEN);
    ...
    if(operation_otz.enc_dec.enc_error_state != OTZ_SUCCESS) {
        ...
        memncpy(output_buf,out_data,output_buf_len,
			CRYPT_BUF_LEN, CRYPT_BUF_LEN);
    }
handle_error_1:
    otz_operation_release(&operation_otz);
...
/**
 * @brief This functions performs the crypto tests 
 *        based on the passed algorithm parameters
 *
 * @param input_buf
 * @param input_buf_len
 * @param init_vector
...
/**
 * @brief MD5 and SHA crypto tests
 */
...
/**
 * @brief Hash based message authentication code Tests 
 */
...
int test_notification()
{
    ...
    memncpy(echo_data.data,"test notification", strlen("test notification")+1,
			CRYPT_BUF_LEN, CRYPT_BUF_LEN);
